...
//JFrame f1=new JFrame("Student Information");
		...
		f1.add(jl2); f1.add(age);
		f1.add(jl3); f1.add(address);
		f1.add(jl4); f1.add(cat);
		f1.add(jl5); f1.add(sgpa1);
		f1.add(jl6); f1.add(sgpa2);
		f1.add(jl7); f1.add(sgpa3);
		f1.add(jl8); f1.add(sgpa4);
		f1.add(jl9); f1.add(sgpa5);
		f1.add(jl0); f1.add(sgpa6);
		f1.add(jl11); f1.add(sgpa7);
		f1.add(jl12); f1.add(sgpa8);
...
//check for validations
			try {
				...
				if (v1 < 18 || v1 > 35) {
					String age1 = JOptionPane.showInputDialog(null, "Enter validAge");
					age.setText(age1);
				}
		...
		else if (evt.getSource() == done)// to submit into collection
		{
			Student s1 = new Student(name.getText(), usn.getText(), address.getText(),
					String.valueOf(cat.getSelectedItem()), Integer.parseInt(age.getText()),
